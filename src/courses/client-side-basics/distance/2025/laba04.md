...
Также есть действие "Дрессировка". Одновременно запускает покормить и поиграть, что увеличивает счастье и энергию. Если какое-либо из действий отклоняется (например, питомец уже был слишком сыт, чтобы есть), все действие завершается отклонением promise.
...
Проведите ручное тестирование в браузере, отформатируйте документы, убедитесь, что нет замечаний от ESLint, зафиксируйте результаты работы с помощью системы контроля версий git и отправьте их в репозиторий на gitverse.
