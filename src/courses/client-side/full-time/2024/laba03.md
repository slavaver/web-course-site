...
Работая над этим заданием вы, создадите SPA веб-приложение на Vue 3 с Composition API и TypeScript, изучив дополнительно маршрутизацию, слоты, shallowRef() и shallowReactive(). Начнете понимать различия между действиями, вычислениями и данными. Попробуете концепции функционального программирования: отображение (map), фильтрация (filter), сокращение (reduce) и неизменяемость.
...
- Никаких изменений в расчетах (копирование при записи для всех операций с данными + shallowRef/shallowReactive). Пример ниже.
