...
Поток — это принцип расположения элементов на странице без использования стилей. Если создать HTML-код и не подключить CSS, браузер отобразит страницу по встроенным правилам. Например, заголовки `<h1>` будут больше, чем `<h2>`, а ссылки будут синими и подчёркнутыми.

Элементы на странице делятся на две группы: блочные (block) и строчные (inline). Например, `<div>` — это блочный элемент, а `<span>` или `<a>` — строчные. Поменять это поведение можно с помощью CSS-свойства `display`.

Если не использовать стили, браузер создаёт нормальный поток элементов. Блочные и строчные элементы отображаются по-разному.
...
то размер элемента будет 200 на 200 пикселей. В идеале не задавать конкретную высоту блока, так как может получится переполнение, т. е. когда контентная часть вылезает за границы блока. По умолчанию высоте задается значение, при котором высота автоматически подстраивается под контент:
